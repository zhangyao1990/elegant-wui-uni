  ...
  <view>
    ...
    <view class="month">
      <view class="wui-month">
        <view class="wui-month__title" v-if="showTitle">{{ monthTitle(date) }}</view>
        <view class="wui-month__days">
          <view
            v-for="(item, index) in days"
            :key="index"
            :class="`wui-month__day ${item.disabled ? 'is-disabled' : ''} ${item.isLastRow ? 'is-last-row' : ''} ${
...
function getFormatterDate(date: number, day: string | number, type?: CalendarDayType) {
  ...
  if (props.formatter) {
    if (isFunction(props.formatter)) {
      ...
      console.error('[elegant-wui-uni] error(wui-calendar-view): the formatter prop of wui-calendar-view should be a function')
    }
  }
  return dayObj
}
