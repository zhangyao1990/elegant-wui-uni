<template>
  <view
    :class="`wui-checkbox ${innerCell ? 'is-cell-box' : ''} ${innerShape === 'button' ? 'is-button-box' : ''} ${isChecked ? 'is-checked' : ''} ${
    ...
    @click="toggle"
  >
    <!--shape为button时，移除wui-checkbox__shape，只保留wui-checkbox__label-->
    <view
      v-if="innerShape !== 'button'"
      :class="`wui-checkbox__shape ${innerShape === 'square' ? 'is-square' : ''} ${customShapeClass}`"
      :style="isChecked && !innerDisabled && innerCheckedColor ? 'color :' + innerCheckedColor : ''"
    >
      <wui-icon custom-class="wui-checkbox__check" name="check-bold" />
    </view>
    <!--shape为button时只保留wui-checkbox__label-->
    <view
      :class="`wui-checkbox__label ${customLabelClass}`"
      :style="isChecked && innerShape === 'button' && !innerDisabled && innerCheckedColor ? 'color:' + innerCheckedColor : ''"
    >
      <!--button选中时展示的icon-->
      <wui-icon v-if="innerShape === 'button' && isChecked" custom-class="wui-checkbox__btn-check" name="check-bold" />
      <!--文案-->
      <view class="wui-checkbox__txt" :style="maxWidth ? 'max-width:' + maxWidth : ''">
        <slot></slot>
      </view>
    </view>
  </view>
...
import { CHECKBOX_GROUP_KEY } from '../wui-checkbox-group/types'
