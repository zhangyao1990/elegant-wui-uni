<template>
  <view :class="`wui-circle ${customClass}`" :style="customStyle">
    <!-- #ifdef MP-WEIXIN -->
    ...
    <view v-if="!text" class="wui-circle__text">
      <!-- 自定义提示内容 -->
      <slot></slot>
    </view>

    <text v-else class="wui-circle__text">
      {{ text }}
    </text>
  </view>
</template>
...
const canvasId = ref<string>(`wui-circle${uuid()}`) // canvasId
