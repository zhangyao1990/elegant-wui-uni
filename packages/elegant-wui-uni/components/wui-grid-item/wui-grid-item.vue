    ...
    <view :class="`wui-grid-item__content ${square ? 'is-square' : ''} ${border && gutter > 0 ? 'is-round' : ''}`" :style="gutterContentStyle">
      ...
      <block v-else>
        <view :style="'width:' + iconSize + '; height: ' + iconSize" class="wui-grid-item__wrapper">
          <wui-badge custom-class="badge" v-bind="customBadgeProps">
            <template v-if="useIconSlot">
              <slot name="icon" />
            </template>
            <wui-icon v-else :name="icon" :size="iconSize" :custom-class="customIcon" />
          </wui-badge>
        </view>
        <slot name="text" v-if="useTextSlot" />
        <view v-else class="wui-grid-item__text custom-text">{{ text }}</view>
      </block>
    </view>
  </view>
</template>
...
import { GRID_KEY } from '../wui-grid/types'
  ...
  if (url) {
    switch (linkType) {
      ...
      case 'switchTab':
        ...
        break
      default:
        console.error(`[wui-design] warning(wui-grid-item): linkType can not be ${linkType}`)
        break
    }
  }
}
