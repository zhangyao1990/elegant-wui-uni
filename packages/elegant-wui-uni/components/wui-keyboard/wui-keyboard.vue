  ...
  >
    <view :class="`wui-keyboard ${customClass}`" :style="customStyle">
      ...
      <template v-if="mode !== 'car'">
        <view class="wui-keyboard__body">
          <view class="wui-keyboard__keys">
            <wui-key v-for="key in keys" :key="key.text" :text="key.text" :type="key.type" :wider="key.wider" @press="handlePress"></wui-key>
          </view>
          <view class="wui-keyboard__sidebar" v-if="mode === 'custom'">
            <wui-key v-if="showDeleteKey" large :text="deleteText" type="delete" @press="handlePress"></wui-key>
            <wui-key large :text="closeText" type="close" :loading="closeButtonLoading" @press="handlePress"></wui-key>
          </view>
        </view>
      </template>
      <template v-if="mode === 'car'">
        <view class="wui-keyboard-car__body">
          <view class="wui-keyboard-car__keys">
            <wui-key v-for="key in keys" :key="key.text" :text="key.text" :type="key.type" :wider="key.wider" @press="handlePress"></wui-key>
          </view>
        </view>
      </template>
