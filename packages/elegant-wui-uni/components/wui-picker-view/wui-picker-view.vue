<template>
  <view :class="`wui-picker-view ${customClass}`" :style="customStyle">
    <view class="wui-picker-view__loading" v-if="loading">
      <wui-loading :color="loadingColor" />
    </view>
    <view :style="`height: ${columnsHeight - 20}px;`">
      <picker-view
        mask-class="wui-picker-view__mask"
        indicator-class="wui-picker-view__roller"
        ...
        @pickend="onPickEnd"
      >
        <picker-view-column v-for="(col, colIndex) in formatColumns" :key="colIndex" class="wui-picker-view-column">
          <view
            v-for="(row, rowIndex) in col"
            :key="rowIndex"
            :class="`wui-picker-view-column__item ${row['disabled'] ? 'wui-picker-view-column__item--disabled' : ''}  ${
              selectedIndex[colIndex] == rowIndex ? 'wui-picker-view-column__item--active' : ''
            }`"
