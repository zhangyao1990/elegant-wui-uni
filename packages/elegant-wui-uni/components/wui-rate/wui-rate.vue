<template>
  <view :class="`wui-rate ${customClass}`" :style="customStyle" @touchmove="onTouchMove">
    <view
    ...
    >
      <wui-icon
        custom-class="wui-rate__item-star"
        :name="isActive(rate) ? activeIcon : icon"
        :size="size"
        :custom-style="rate === '100%' ? iconActiveStyle : iconStyle"
        @click="changeRate(index, false)"
      />
      <view v-if="props.allowHalf" class="wui-rate__item-half" @click.stop="changeRate(index, true)">
        <wui-icon
          custom-class="wui-rate__item-star"
          :name="isActive(rate) ? activeIcon : icon"
          :size="size"
...
function computeRateList() {
  ...
  if (typeof modelValue !== 'number') {
    console.error('[Elegant Wui Uni] error(wui-rate): the value of wui-rate should be a number')
    return
  }
...
async function onTouchMove(event: TouchEvent) {
  ...
  const rateItems = await getRect('.wui-rate__item', true, proxy)
