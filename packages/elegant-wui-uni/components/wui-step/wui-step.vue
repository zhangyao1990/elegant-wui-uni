<template>
  <view
    v-if="currentStatus"
    :class="`wui-step ${customClass} ${currentStatus ? 'is-' + currentStatus : ''} ${canAlignCenter ? 'is-center' : ''}  ${
      vertical ? 'is-vertical' : ''
    }`"
    :style="rootStyle"
  >
    <view :class="`wui-step__header  ${dot ? 'is-dot' : ''}`">
      <view :class="`wui-step__icon  ${dot ? 'is-dot' : !!icon || $slots.icon ? 'is-icon' : 'is-text'}`">
        <view v-if="dot" class="wui-step__dot"></view>
        <slot v-else-if="$slots.icon" name="icon" />
        <wui-icon v-else-if="icon" custom-class="wui-step__icon-inner" :name="icon" />
        <view v-else class="wui-step__icon-outer">
          <wui-icon v-if="currentStatus === 'finished'" name="check-bold" />
          <wui-icon v-else-if="currentStatus === 'error'" name="close-bold" />
          <text v-else>{{ index + 1 }}</text>
        </view>
      </view>
      <view v-if="index < childrenLength - 1" class="wui-step__line"></view>
    </view>
    <view class="wui-step__content">
      <view :class="`wui-step__title ${$slots.description || description ? 'is-description' : ''}`">
        <slot v-if="$slots.title" name="title" />
        <text v-else>{{ currentTitle }}</text>
      </view>
      <view v-if="$slots.description || description" class="wui-step__description">
...
import { STEPS_KEY } from '../wui-steps/types'
