<template>
  ...
  <view
    :class="`wui-swipe-action ${customClass}`"
    :style="customStyle"
    @click.stop="onClick()"
  ...
  >
    <!--容器-->
    <view class="wui-swipe-action__wrapper" :style="wrapperStyle">
      <!--左侧操作-->
      <view class="wui-swipe-action__left" @click="onClick('left')">
        <slot name="left" />
      </view>
      <!--内容-->
      <slot />
      <!--右侧操作-->
      <view class="wui-swipe-action__right" @click="onClick('right')">
        <slot name="right" />
      </view>
    </view>
  </view>
...
function getWidths() {
  return Promise.all([
    getRect('.wui-swipe-action__left', false, proxy).then((rect) => {
      return rect.width ? rect.width : 0
    }),
    getRect('.wui-swipe-action__right', false, proxy).then((rect) => {
      return rect.width ? rect.width : 0
    })
  ])
}
