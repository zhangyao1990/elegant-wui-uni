<template>
  <view :class="{ 'wui-tabbar__placeholder': fixed && placeholder && safeAreaInsetBottom && shape === 'round' }" :style="{ height: addUnit(height) }">
    <view
      :class="`wui-tabbar wui-tabbar--${shape} ${customClass} ${fixed ? 'is-fixed' : ''} ${safeAreaInsetBottom ? 'is-safe' : ''} ${
        bordered ? 'is-border' : ''
      }`"
      :style="rootStyle"
    >
...
import type { TabbarItem } from '../wui-tabbar-item/types'
...
function setPlaceholderHeight() {
  ...
  getRect('.wui-tabbar', false, proxy).then((res) => {
