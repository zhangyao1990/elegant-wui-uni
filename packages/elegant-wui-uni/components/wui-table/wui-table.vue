  ...
  <view :class="`wui-table ${border ? 'is-border' : ''} ${customClass}`" :style="tableStyle">
    <template v-if="fixedHeader">
      <scroll-view
        ...
        v-if="showHeader"
      >
        <view id="table-header" class="wui-table__content wui-table__content--header" :style="realWidthStyle">
          <view
    ...
    <template v-else>
      <scroll-view class="wui-table__wrapper" :enable-flex="true" :throttle="false" :scroll-x="true" @scroll="scroll" :scrollLeft="state.scrollLeft">
        <view class="wui-table__inner" :style="realWidthStyle">
          <!-- 表头部分 -->
          <view v-if="showHeader" class="wui-table__header-row">
            <view
              v-for="(column, index) in children"
              :key="index"
              :class="`wui-table__cell ${border ? 'is-border' : ''} ${column.fixed ? 'is-fixed' : ''} ${stripe ? 'is-stripe' : ''} is-${
                column.align
              } ${getIsLastFixed(column) && state.scrollLeft ? 'is-shadow' : ''}`"
              :style="getCellStyle(index)"
            >
          ...
          <!-- 表格内容部分 -->
          <view class="wui-table__content" :style="bodyStyle">
            <wui-table-col
              v-if="index !== false"
              :prop="indexColumn.prop"
              :label="indexColumn.label"
