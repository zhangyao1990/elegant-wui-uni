...
const extractSCSSVariables = (scssFilePath: string): Record<string, string> => {
  ...
  while ((match = variableRegex.exec(scssContentToProcess)) !== null) {
    const keyComment = match[1].replace(/-([a-z])/g, (match: any, letter: any) => letter.toUpperCase())
    variables[keyComment] = match[2].trim()
  }
  return variables
}
 ...
 */
const generateTSFileContent = (variables: Record<string, string>):string => {
...
const tsFilePath = path.resolve(__dirname, '../src/uni_modules/elegant-wui-uni/components/wui-config-provider/types.ts')
const scssFilePath = path.resolve(__dirname, '../src/uni_modules/elegant-wui-uni/components/common/abstracts/variable.scss')
