<template>
  <view :style="customStyle" :class="`wui-drop-menu ${customClass}`" @click.stop="noop" :id="dropMenuId">
    <!-- #ifdef MP-DINGTALK -->
    <view :id="dropMenuId">
      <!-- #endif -->
...
watch(
  ...
  (newValue) => {
    if (!['up', 'down'].includes(newValue)) {
      ...
      console.error("[wui design] warning(wui-drop-menu): direction must be 'up' or 'down'")
    }
  },
...
function getDisplayTitle(child: any) {
  ...
  console.error('[wui-design] warning(wui-drop-menu-item): no value is matched in the options option.')
}
