<template>
  <view :style="customStyle" :class="`wui-drop-menu ${customClass}`" @click.stop.prevent="noop" :id="dropMenuId">
    <wui-overlay :show="overlayVisible" :duration="duration" :z-index="12" :custom-style="modalStyle" @click="handleClickOverlay" @touchmove="noop" />

    <!-- #ifdef MP-DINGTALK -->
    <view :id="dropMenuId">
      <!-- #endif -->
      <view class="wui-drop-menu__list">
        <view
          v-for="(child, index) in children"
          :key="index"
          @click="toggle(child)"
          :class="`wui-drop-menu__item ${child.disabled ? 'is-disabled' : ''} ${child.$.exposed!.getShowPop() ? 'is-active' : ''}`"
        >
          <view class="wui-drop-menu__item-title">
            <view class="wui-drop-menu__item-title-text">{{ getDisplayTitle(child) }}</view>
            <wui-icon :name="child.icon" :size="child.iconSize" custom-class="wui-drop-menu__arrow" />
          </view>
        </view>
      </view>
      <slot />
...
watch(
  ...
  (newValue) => {
    if (!['up', 'down'].includes(newValue)) {
      ...
      console.error("[elegant-wui-uni] warning(wui-drop-menu): direction must be 'up' or 'down'")
    }
  },
...
function getDisplayTitle(child: any) {
  ...
  console.error('[elegant-wui-uni] warning(wui-drop-menu-item): no value is matched in the options option.')
}
