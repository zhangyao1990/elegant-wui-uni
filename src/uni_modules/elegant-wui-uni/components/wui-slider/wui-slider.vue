<template>
  <view :class="rootClass" :style="customStyle">
    ...
    <view style="flex: 1" :class="rootClass">
      <!-- #endif -->
      <view :class="`wui-slider__label-min ${customMinClass}`" v-if="!hideMinMax">{{ minProp }}</view>
      <view class="wui-slider__bar-wrapper" :style="wrapperStyle" :id="sliderBarWrapperId">
        <view class="wui-slider__bar" :style="barStyle">
          <template v-if="isRange">
            <!-- 左边滑块 -->
            <view
              class="wui-slider__button-wrapper wui-slider__button-wrapper--left"
            ...
            <!-- 右边滑块 -->
            <view
              class="wui-slider__button-wrapper wui-slider__button-wrapper--right"
              :style="sliderButtonStyle"
              ...
              @touchcancel="onSliderTouchEnd"
            >
              <view class="wui-slider__label" v-if="!hideLabel">{{ secondValue }}</view>
              <view class="wui-slider__button" />
            </view>
          </template>
          <view
...
const stepProp = computed(() => {
  if (props.step <= 0) {
    console.warn('[elegant-wui-uni] warning(wui-slider): step must be greater than 0')
    return 1
  }
  return props.step
})
...
const rootClass = computed(() => {
  return `wui-slider ${!props.hideLabel ? 'wui-slider__has-label' : ''} ${props.disabled ? 'wui-slider--disabled' : ''} ${props.customClass}`
})
...
function normalizeRangeValues(value: number[]): [number, number] {
  ...
  if (!Array.isArray(value) || value.length < 2) {
    console.warn('[elegant-wui-uni] warning(wui-slider): range value should be an array with at least 2 elements')
