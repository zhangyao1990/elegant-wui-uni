<template>
  <view :class="rootClass" :style="customStyle" :id="sliderId">
    ...
    <view :id="sliderId" style="flex: 1" :class="rootClass">
      <!-- #endif -->
      <view :class="`wui-slider__label-min ${customMinClass}`" v-if="!hideMinMax">
        {{ minValue }}
      </view>
      <view class="wui-slider__bar-wrapper" :style="barWrapperStyle">
        <view class="wui-slider__bar" :style="barCustomStyle"></view>
        <!-- 左边 -->
        <view
          class="wui-slider__button-wrapper"
        ...
        >
          <view class="wui-slider__label" v-if="!hideLabel">{{ leftNewValue }}</view>
        ...
        >
          <view class="wui-slider__label" v-if="!hideLabel">{{ rightNewValue }}</view>
          <view class="wui-slider__button" />
        </view>
      </view>
      <view :class="`wui-slider__label-max ${customMaxClass}`" v-if="!hideMinMax">
        {{ maxValue }}
      </view>
      <!-- #ifdef MP-DINGTALK -->
    </view>
...
watch(
  ...
  (newValue) => {
    if (newValue <= props.min) {
      ...
      console.warn('[wui design] warning(wui-slider): max value must be greater than min value')
    } else {
...
watch(
  ...
  (newValue) => {
    if (newValue >= props.max) {
      ...
      console.warn('[wui design] warning(wui-slider): min value must be less than max value')
    } else {
...
watch(
  ...
  (newValue) => {
    if (newValue < 1) {
      stepValue.value = 1
      console.warn('[wui design] warning(wui-slider): step must be greater than 0')
    } else {
...
watch(
  ...
  (newValue) => {
    ...
    if (newValue === null || newValue === undefined) {
      ...
      console.warn('[wui design] warning(wui-slider): value can nott be null or undefined')
    } else if (isArray(newValue) && newValue.length !== 2) {
      console.warn('[wui design] warning(wui-slider): value must be dyadic array')
    } else if (!isNumber(newValue) && !isArray(newValue)) {
      emit('update:modelValue', currentValue.value)
      console.warn('[wui design] warning(wui-slider): value must be dyadic array Or Number')
    }
    // 动态传值后修改
    if (isArray(newValue)) {
...
const rootClass = computed(() => {
  const rootClass = `wui-slider ${!props.hideLabel ? 'wui-slider__has-label' : ''} ${props.disabled ? 'wui-slider--disabled' : ''} ${
    props.customClass
  }`
  return rootClass
})
