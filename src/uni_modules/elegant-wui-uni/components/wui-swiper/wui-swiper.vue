<template>
  <view :class="`wui-swiper ${customClass}`" :style="customStyle">
    ...
    <scroll-view scroll-x scroll-y style="width: 100%; height: 100%">
      ...
      >
        <swiper-item v-for="(item, index) in list" :key="index" class="wui-swiper__item">
          <video
            v-if="isVideo(item)"
            :id="`video-${index}-${uid}`"
            :style="{ height: addUnit(height) }"
            ...
            :class="`wui-swiper__video ${customItemClass} ${getCustomItemClass(currentValue, index, list)}`"
          ...
          <image
            v-else
            :src="isObj(item) ? item[valueKey] : item"
            :class="`wui-swiper__image ${customImageClass} ${customItemClass} ${getCustomItemClass(currentValue, index, list)}`"
            :style="{ height: addUnit(height) }"
            :mode="imageMode"
            @click="handleClick(index, item)"
          />
